function [Fidel,Gradient]=fidelity(initial_state,target_state,target_unitary,dt,N,phi_theta,F,A,lam,casefidelity)
    ...
    %This function calculates both fidelity and gradient of fidelity with hamiltonian given by the function Hamiltonian(...).
    ...
    for ii=num:-1:1
        ...
        dH_dtheta=dHamiltonian_dtheta(N,theta(ii),phi(ii),F,A,lam);
        
        dU_dphi(:,:,ii)=V * ((V' * dH_dphi * V).*prefac) * V';    
        dU_dtheta(:,:,ii)=V * ((V' * dH_dtheta * V).*prefac) * V';
    ...
    if nargout>1
        ...
        for ii=1:num   
            ...
            if casefidelity==1
                ...
                Gradient(num+ii,1)= -2 * real((target_state'*U*initial_state)' * target_state'* UU2*initial_state);                
            elseif casefidelity==2
                Gradient(ii,1)= -2 * (1/(4*N^2))*(real(trace(target_unitary'*UU1)*conj(a)));
                Gradient(num+ii,1)= -2 * (1/(4*N^2))*(real(trace(target_unitary'*UU2)*conj(a)));
            end
        end
    end
end  
